...
export default function Skills() {
  ...
  const demo = useMemo(() => {
    return codeDemos[currentDemo]
  }, [currentDemo]);

  useEffect(() => {
    setTimeout(() => {
      setCurrentDemo(currentDemo + 1 == codeDemos.length ? 0 : currentDemo + 1);
    }, 30000);
  }, [demo]);
    ...
    <Linked id="skills" className="bg-slate-300 dark:bg-neutral-900! pb-10">
      <div>
        ...
        <div className="grid md:grid-cols-2 gap-2">
          ...
          <div className="font-medium text-2xl mb-4 break-normal dark:text-white text-title">
          ...
          <p className="text-sm text-gray-600 dark:text-slate-100 leading-6">
            <ul className="list-disc list-inside">
              {topSkill.details.map((detail, index) => (
                <li key={"top-skill-detail" + index} className="mb-2">{detail}</li>
              ))}
        ...
        <div className="my-10">
          <div className="p-4 rounded-lg flex flex-wrap gap-4 my-10">
            {myAllStacks.map((stack, index) => (
              <div key={"stack" + index} className="flex items-center justify-center gap-2 rounded-lg p-4 shadow-lg bg-white dark:bg-neutral-800 hover:bg-blue-500 dark:text-white hover:text-white dark:hover:bg-blue-500 dark:hover:text-white transition-all duration-300">
                {stack.icon}
                {stack.label}
              </div>
            ))}
        ...
        {demo && (
            <div className=" max-w-5xl px-4 overflow-hidden transition duration-500  mx-auto">
              <h6 className="md:text-lg my-4 font-medium mb-2 md:my-8 dark:text-white/80 underline">{t("feature-examples")}</h6>
              <div className="m-4 text-center">
                
                    <h1 className="text-base! md:text-lg! font-medium! text-gray-500 dark:text-white/50">
                      {t(demo.title)}
                    </h1>
