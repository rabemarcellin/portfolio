...
export default function Skills() {
  ...
  return (
    <Linked id="skills" className="dark:bg-slate-700 pb-10">
      <div  className="max-w-5xl mx-auto px-4">
      <h6 className="title ">{t("skill-title") }</h6>
      <div className="grid md:grid-cols-2 gap-10">
        {myTopSkills.map((topSkill, index) => (
          <div key={"top-skill" + index} className="col-span-1 bg-white/75 dark:bg-slate-900 hover:bg-white shadow hover:shadow-lg transition-shadow duration-300 border-2 border-black dark:border-inherit rounded-lg p-4">
        ...
        <div className="font-bold text-4xl mb-4 break-normal">
          <span className="text-blue-500 mr-2">{"->"}</span>
          {topSkill.title}</div>
        <div className="border-b border-gray-500 w-fit mb-4">
        <h4 className="ml-2 text-gray-500">{topSkill.metadata}</h4>
        </div>
      ...
      <div className="my-10">
        <div className="p-4 rounded-lg flex flex-wrap gap-4 my-10">
          {myAllStacks.map((stack, index) => (
            <div key={"stack" + index} className="flex items-center justify-center gap-2 rounded-lg p-4 shadow-lg bg-white dark:bg-slate-900 hover:bg-blue-500 hover:text-white dark:hover:bg-blue-500 dark:hover:text-white transition-all duration-300">
              {stack.icon}
              {stack.label}
            </div>
          ))}
